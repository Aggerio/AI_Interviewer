...
import { derived, waitForState, keepAlive, autorunHandleChanges, transaction, observableValue } from 'monaco-editor/esm/vs/base/common/observable.js';
...
let MergeEditorModel = class MergeEditorModel extends EditorModel {
    constructor(base, input1, input2, resultTextModel, diffComputer, options, telemetry, languageService, undoRedoService) {
        ...
        this.modifiedBaseRanges = derived('modifiedBaseRanges', (reader) => {
        ...
        this.modifiedBaseRangeResultStates = derived('modifiedBaseRangeResultStates', reader => {
        ...
        this.input1ResultMapping = derived('input1ResultMapping', reader => {
        ...
        this.resultInput1Mapping = derived('resultInput1Mapping', reader => this.input1ResultMapping.read(reader).reverse());
        this.input2ResultMapping = derived('input2ResultMapping', reader => {
        ...
        this.resultInput2Mapping = derived('resultInput2Mapping', reader => this.input2ResultMapping.read(reader).reverse());
        this.baseResultMapping = derived('baseResultMapping', reader => {
        ...
        this.resultBaseMapping = derived('resultBaseMapping', reader => this.baseResultMapping.read(reader).reverse());
        this.diffComputingState = derived('diffComputingState', reader => {
            const states = ( [
        ...
        this.inputDiffComputingState = derived('inputDiffComputingState', reader => {
            const states = ( [
                this.input1TextModelDiffs,
                this.input2TextModelDiffs,
            ].map((s) => s.state.read(reader)));
        ...
        this.isUpToDate = derived('isUpToDate', reader => this.diffComputingState.read(reader) === 2 );
        this.onInitialized = waitForState(this.diffComputingState, state => state === 2 ).then(() => { });
        this.firstRun = true;
        this.unhandledConflictsCount = derived('unhandledConflictsCount', reader => {
...
class ModifiedBaseRangeData {
    constructor(baseRange) {
        ...
        this.handled = derived('handled', reader => this.handledInput1.read(reader) && this.handledInput2.read(reader));
    }
}
