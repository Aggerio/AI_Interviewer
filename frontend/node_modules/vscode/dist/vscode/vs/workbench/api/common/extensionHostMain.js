...
import { InstantiationService } from 'monaco-editor/esm/vs/platform/instantiation/common/instantiationService.js';
...
class ExtensionHostMain {
    ...
    async getAllStaticBrowserUris() {
        const mainThreadExtensionsProxy = ( (this._rpcProtocol.getProxy(MainContext.MainThreadExtensionService)));
        return (
             ((await mainThreadExtensionsProxy.$getAllStaticBrowserUris()).map(([from, to]) => [URI.revive(from), URI.revive(to)]))
        );
    }
    terminate(reason) {
