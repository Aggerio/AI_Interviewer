...
var cssParser = getUrls;

var parseCssUrls = cssParser;
var cssUrlParser = parseCssUrls;

var parseCssUrl = /*@__PURE__*/getDefaultExportFromCjs(cssUrlParser);
...
function extractCommandResources(command) {
...
function extractGrammarResources(grammar) {
    return [{ path: grammar.path }];
}
function extractLanguageResources(language) {
    const resources = [];
...
function extractSnippetsResources(snippet) {
    return [{ path: snippet.path, mimeType: 'application/json' }];
}
async function extractThemeResources(theme, getFileContent) {
    const themeContent = await getFileContent(theme.path);
    const themeDocument = parseJson(theme.path, themeContent.toString('utf8'));
    const paths = [{ path: theme.path, mimeType: 'application/json' }];
...
function extractJsonValidationResources(jsonValidation) {
...
async function extractResourcesFromExtensionManifestContribute(contribute, getFileContent) {
    const resources = [];
    ...
    if (contribute.themes != null)
        resources.push(...((await Promise.all(contribute.themes.map(theme => extractThemeResources(theme, getFileContent), getFileContent))).flat()));
    if (contribute.iconThemes != null)
        resources.push(...((await Promise.all(contribute.iconThemes.map(theme => extractThemeResources(theme, getFileContent), getFileContent))).flat()));
    if (contribute.productIconThemes != null)
        resources.push(...((await Promise.all(contribute.productIconThemes.map(theme => extractThemeResources(theme, getFileContent), getFileContent))).flat()));
...
async function extractResourcesFromExtensionManifest(manifest, getFileContent) {
    let resources = [];
    if (manifest.contributes != null) {
        resources.push(...await extractResourcesFromExtensionManifestContribute(manifest.contributes, getFileContent));
    }
    ...
    resources = resources.map(r => {
        if (r.mimeType == null) {
            const detectedType = mimeTypes.lookup(r.path);
...
async function extractResources(resourcePath, getFileContent) {
    const resources = [];
...
export { extractResourcesFromExtensionManifest as e, localizeManifest as l, parseJson as p };
